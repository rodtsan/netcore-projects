...
namespace PinoyCode.Cqrs
{
    ...
    public class ESEventStore : IEventStore
    {
        ...
        public IEnumerable LoadEventsFor<TAggregate>(Guid id)
        {
            ...
            do
            {
                currentSlice = Task<StreamEventsSlice>.Run(() =>
                {
                    return conn.ReadStreamEventsForwardAsync(id.ToString(), nextSliceStart, 200, false);
                }).Result;
        ...
        public void SaveEventsFor<TAggregate>(Guid? id, int eventsLoaded, ArrayList newEvents)
        {
            ...
            Task<WriteResult>.Run(() =>
            {
                 conn.AppendToStreamAsync(aggregateId.ToString(), expected, newEvents
                .Cast<dynamic>()
